...
import randomforest; reload(randomforest); from randomforest import *
...
def roc_plot():
    ...
    model = RandomForestClassifier(n_estimators=300, max_depth=15, min_samples_split=0.1, oob_score=True, random_state=4)
    i = 0
    tprs = []
    aucs = []
...
def main():
  # Bayesian Optimization
    ...
    fi_model = pd.DataFrame({'feature': features,'importance': importances}).sort_values('importance', ascending = False)
    ...
    labels = fi_model['feature']
    labels = [ '\n'.join(wrap(l, 20)) for l in labels ]
    # Plot the feature importances of the forest
    plt.figure(6)
    plt.title("Feature importances", fontsize=30)
    plt.bar(range(X_plot.shape[1]), importances[indices],
               color="b")
    plt.xlabel('Feature', fontsize=25)
    plt.ylabel('Importance in %', fontsize=25)
    plt.xticks(range(X_plot.shape[1]), labels, rotation=70, fontsize = 20)
